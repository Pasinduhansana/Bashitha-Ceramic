...
function groupLogsBySection(logs) {
  ...
  for (const log of logs) {
    ...
    let label = ts >= startOfToday ? "TODAY" : ts >= startOfYesterday ? "YESTERDAY" : ts.toLocaleDateString(undefined, { year: "numeric", month: "short", day: "numeric" });
...
export default function Activities() {
  ...
  return (
    <div className="px-6 py-6 min-h-screen bg-white dark:bg-gray-900 transition-colors">
      {/* Header */}
      <div className="mb-6">
        <h2 className="text-xl font-bold text-gray-900 dark:text-white mb-1">Activity History</h2>
      ...
      <div className="mb-6 flex items-center justify-between">
        <div className="flex items-center gap-6">
          ...
          {/* User Filter */}
          <div className="relative flex items-center gap-2">
            <span className="text-xs font-semibold text-gray-700 dark:text-gray-300">User :</span>
            <button
            ...
            {userOpen && (
              <div className="absolute top-full z-20 mt-2 w-40 rounded-lg border-2 border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 shadow-xl p-1">
                {["Admin", "All Users", "You", "Suhansek", "KasimiraS"].map((opt) => (
                  <button
                    key={opt}
                    className={`flex w-full items-center gap-2 rounded px-3 py-2 text-xs font-medium ${
                      userFilter === opt
        ...
        <div className="relative">
          ...
          <input
            ...
            className="w-64 rounded-lg border-2 border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 py-2 pl-10 pr-4 text-xs text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-gray-500 outline-none focus:border-[#1fb8a2] focus:ring-2 focus:ring-[#1fb8a2]/20 transition-all"
          />
        </div>
      </div>
      ...
      <div className="space-y-6">
        ...
        {!loading && groupedSections.map((section) => (
          <div key={section.section}>
            {/* Section Header */}
            <button className="mb-4 flex items-center gap-2 rounded-full bg-linear-to-r from-[#1fb8a2] to-[#17a694] px-4 py-2 text-xs font-bold text-white shadow-md hover:shadow-lg transition-all">
              {section.section}
              ...
              {section.items.map((log, activityIndex) => (
                <div key={log.id} className="relative pl-16">
                  ...
                  {activityIndex !== section.items.length - 1 && (
                    <div className="absolute left-6 top-12 bottom-0 w-0.5 bg-gray-200 dark:bg-gray-700" style={{ height: "calc(100% + 1.5rem)" }} />
                  )}

                  {/* Avatar */}
                  <div className="absolute left-0 top-0 z-10">
                    <div className="flex h-12 w-12 items-center justify-center rounded-full bg-linear-to-br from-[#1fb8a2] to-[#17a694] text-white font-bold text-xs border-4 border-white dark:border-gray-900 shadow-lg">
                  ...
                  <div>
                    <div className="mb-2">
                      <h3 className="text-xs font-bold text-gray-900 dark:text-white">{log.details}</h3>
                      <p className="text-[10px] text-gray-500 dark:text-gray-400">
                        {new Date(log.timestamp).toLocaleString()} by {log.user_name || "Unknown"}
                      </p>
                    </div>

                    {/* Optional: For certain actions, show a card-style preview (future enhancement) */}
                  </div>
                </div>
              ))}
            </div>
