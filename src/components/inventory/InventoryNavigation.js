...
export default function InventoryNavigation({ active, onChange, userPermissions = [], products = [], onProductSelect }) {
  ...
  return (
    <>
      ...
      <nav className="hidden md:block border-b border-neutral-200 dark:border-gray-800 bg-white dark:bg-gray-900 px-4 lg:px-6 transition-colors">
        <div className="flex items-center gap-1 lg:gap-2 overflow-x-auto">
          {visibleNavItems.map((item) => (
            <button
              key={item.label}
              onClick={() => handleNavClick(item)}
              className={`relative flex items-center gap-1.5 lg:gap-2 h-11 px-2.5 lg:px-3 text-[13px] font-semibold whitespace-nowrap transition-colors ${
                active === item.label ? "text-brand-800 dark:text-brand-400" : "text-neutral-500 dark:text-gray-400 hover:text-neutral-800 dark:hover:text-gray-200"
              }`}
            >
              <item.icon className="h-3.5 w-3.5 shrink-0" />
              <span>{item.label}</span>
              {active === item.label && <span className="absolute bottom-0 left-0 right-0 h-[2px] bg-brand-700 rounded-full" />}
            </button>
          ))}
        </div>
      </nav>
